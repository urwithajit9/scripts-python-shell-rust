...
def parse_last():
    ...
    for boot_time, shutdown_time in BOOT_RE.findall(out):
        ...
        if "still running" in out:
            ...
            st = datetime.strptime(shutdown_time, "%a %b %d %H:%M:%S %Y") if shutdown_time else None
    ...
    for boot, shutdown in sessions:
        ...
        if day not in days:
            ...
            if boot < days[day]["boot"]:
                days[day]["boot"] = boot
            # latest shutdown
            if shutdown and (days[day]["shutdown"] is None or shutdown > days[day]["shutdown"]):
...
def to_csv(days, csv_path="boot_shutdown.csv"):
    ...
    for d, v in days.items():
        rows.append([d.isoformat(), v["boot"].time().isoformat(), v["shutdown"].time().isoformat()])
...
def plot(df):
    ...
    plt.plot(df["date"], df["boot_dt"].dt.hour + df["boot_dt"].dt.minute/60, marker='o')
    plt.plot(df["date"], df["shutdown_dt"].dt.hour + df["shutdown_dt"].dt.minute/60, marker='o')
    plt.xlabel("Date")
